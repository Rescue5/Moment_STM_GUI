...
local function printTelemetry(t)
   ...
   local power = t.MotorI * 0.9514 * t.MotorU
   local mech_power =  ((moment * t.MotorRPM * 1000.0)/9549.0) / lopasti
   local kpd = mech_power/power
   local str = string.format(
      'Скорость:%d:Тяга:%d:Об/мин:%d',
      t.Throttle,
...
local function test(t)
   ...
   local pulseMax = intParam('pulseMax', 2100)
   local pulseInc = intParam('pulseInc',   25)

   -- Список значений газа
   local throttleValues = {1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900, 2000, 2100, 2200}
