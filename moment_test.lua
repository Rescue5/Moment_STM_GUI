...
local scaleFactors = {-425.0, 1.0, 1.0} -- По умолчанию установлены коэффициенты 100
...
local function printTelemetry(t)
   local str = string.format(
      'Скорость:%d:Момент:%.02f:Тяга:%.02f:Об/мин:%d:Ток:%.02f:Напряжение:%.02f:Мощность:%.02f',
      t.Throttle,
      (applyScale(t.Load2, 2) + applyScale(t.Load3, 3)) / 2, -- Средний момент
